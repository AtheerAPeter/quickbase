  ...
  return new Promise((resolve) => {
    ...
    const render = () => {
      ...
      packageManagers.forEach((pm, i) => {
        if (i === selectedIndex) {
          console.log(chalk.green.bold(`> ${pm}`));
        } else {
          console.log(chalk.white(`  ${pm}`));
        }
      });
    };
...
const getDatabase = () => {
  return new Promise((resolve) => {
    ...
    const render = () => {
      ...
      databaseOptions.forEach((db, i) => {
        if (i === selectedIndex) {
          console.log(chalk.green.bold(`> ${db}`));
        } else {
...
const createProjectStructure = async (projectName) => {
  ...
  execCommand(initScripts[packageManager]);
  ...
  execCommand(`${packageManager} add ${dependencies.join(" ")}`);

  console.log(chalk.blue("Installing dev dependencies..."));
  const devFlag = packageManager === "npm" ? "--save-dev" : "-D";
  execCommand(`${packageManager} add ${devFlag} ${devDependencies.join(" ")}`);
